...
for i in range(len(sam.individuals_population)):
    if sam.individuals_population[i]==25:
        ITUlist.append(i)
observed_data = sam.subset(individuals=ITUlist)

print("done finding observed data")

sitefreq = numpy.sum(observed_data.sites_genotypes[0:],1)
siteuse =  numpy.where( sitefreq>0 )[0]  
print("siteuse_rea",len(siteuse))

datause = observed_data.subset(sites = siteuse)

print("done finding datause")

####### observe #########
mts1 = tsinfer.infer(datause,path_compression=False)

print("done infering TS")


name = "ITU_Chr20_mts1_approach12.dat"
file = open(name,"wb") 
pickle.dump(mts1,file)
file.close()
##################################################################
# #################################################################
